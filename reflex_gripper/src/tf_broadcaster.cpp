...
#include <reflex_gripper_msgs/Hand.h>


void poseCallback(const reflex_gripper_msgs::HandConstPtr& msg) {
...
int main(int argc, char** argv){
  ...
  if (argc != 2){ROS_ERROR("need hand name as argument"); return -1;};
  std::string hand_name = argv[1];

  ros::NodeHandle node;
  ros::Subscriber sub = node.subscribe("/reflex_"+hand_name, 10, &poseCallback);
